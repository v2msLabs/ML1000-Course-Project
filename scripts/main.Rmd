...
model.0 <- glm(DEFAULT~ .,data=ccTrain3, family=binomial(link="logit"))
summerizeModel(model.0)

checkModel <- model.0
pred <- predict(checkModel, newdata = ccValidate3, type="response")
pR2(checkModel)
str(pred) 
summary(pred)  
...
missClassifiedError <- mean(fitted.results != ccValidate3$DEFAULT) 
print(paste('Accuracy',1-missClassifiedError))  
pr <- ROCR::prediction(pred, ccValidate3$DEFAULT) 
prf <- ROCR::performance(pr, measure = "tpr", x.measure = "fpr") 
plot(prf)  
auc <- ROCR::performance(pr, measure = "auc") 
#auc <- auc@y.values[[1]] need to work on this from 384 to 387
#auc   
#model.result.11 <-  table(fitted.results, ccValidate3$DEFAULT) 
#model.result.11
