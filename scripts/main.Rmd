...
#randomForestModel = caret::train(as.formula('DEFAULT ~ .'), 
   #data = ccTrain3, method = "rf", metric="ROC", trControl = ctrl)
#proc.time() - ptm_rf
#pred.randomForestModel.prob = predict(randomForestModel, newdata = ccTest3, type="prob")
#pred.randomForestModel.raw = predict(randomForestModel, newdata = ccTest3 )

#roc.randomForestModel = pROC::roc(ccTest3$DEFAULT,  
                                  #as.vector(ifelse(pred.randomForestModel.prob[,"yes"] >0.5, 1,0)) )
#auc.randomForestModel = pROC::auc(roc.randomForestModel)
#randomForestModel
```
```{r}
#confusionMatrix(data = pred.randomForestModel.raw, ccTest3$DEFAULT)
```
