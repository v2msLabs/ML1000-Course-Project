...
* The majory of the credit card holders maintain good standing (see Figure: \ref{fig:payment_status}). 
...
All models we are going to fit will benefit from the data scaling and centering, thus we are going to upply these transformations.
...
Well the result of the model fitted with the PCA componenets is inferiour to the same very model, whichwas fitted with the whole data set by about 7%. But we shall not forget that the PCA data set had less than a half oof the features of the original data set.
...
# Undertanding the Clinet Base Employing Unsupervised Learning

Lastly we believe it would be beneficial to profie the client base. This would add addtional insights into understanding of the credit card holders demographics and spending and borrowing habbits. We will be employing CLARA approach to attack this challange. This study does not make its goal to compare various unsupervised model techniques. CLARA was chozen because it is robust, relatively easy to understand and fast. After many trials and errors we have selected a few features that describe the client financial and demographic profile quite well
...
dataSampleScaled = sample_n(dataC,3000) %>% scale()
H =  get_clust_tendency(dataSampleScaled, n = 100, graph = F, seed = 6701)
...
```{r cluster1, echo=FALSE, fig.align="center", fig.cap="Cluster One", warning=FALSE}
...
p3 = ggplot(cLimit, aes(x="", y=COUNT, fill=CREDIT_LIMIT))+
geom_bar(width = 1, stat = "identity") + coord_polar("y", start=0)

p4 = ggplot(cEd, aes(x="", y=COUNT, fill=Education))+
geom_bar(stat = "identity") +   coord_polar("y", start=0)

p5 = ggplot(cAge, aes(x="", y=COUNT, fill=AGE_GROUP))+
geom_bar(width = 1, stat = "identity") +   coord_polar("y", start=0)

p6 = ggplot(cDefault, aes(x="", y=COUNT, fill=DO_DEFAULT))+
geom_bar(width = 1, stat = "identity" ) + scale_fill_manual(values = c(mainPalette[6], mainPalette[3]))

grid.arrange(p1,p2,p3,p4,p5,p6, ncol = 2)
```

```{r cluster2, echo=FALSE, fig.align="center", fig.cap="Cluster Two", warning=FALSE}
...
```{r cluster3, echo=FALSE, fig.align="center", fig.cap="Cluster Three", warning=FALSE}
...
```{r cluster4, echo=FALSE, fig.align="center", fig.cap="Cluster Four", warning=FALSE}
